...
const block = (to, data, at) => {
    ...
    z = 0;
    do
        a = i + (r(f, 6) ^ r(f, 11) ^ r(f, 25)) + (f & g ^ ~f & h) + A[z] + C[z],
            j = (r(b, 2) ^ r(b, 13) ^ r(b, 22)) + (b & c ^ b & d ^ c & d), i = h, h = g,
            g = f, f = e + a, e = d, d = c, c = b, b = a + j;
    while (++z < 64);
